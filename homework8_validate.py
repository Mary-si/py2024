...
# Предусмотреть защиту от ввода букв, пустой строки и т.д. Примечания Алгоритм Луна
...
def validate_number(card):
    ...
    for index, num in enumerate(card_numbers):
        # если index четный, то число стоит на нечетной позиции, тк считается с нуля
        # цифры стоящие на нечетных местах умножаем на 2
        if index % 2 == 0:
            double_numb = num * 2
            # если есть удвоенное число и оно больше 9, то вычитаем из него 9 и прибавляем к сумме чисел
            if double_numb > 9:
                double_numb -= 9
